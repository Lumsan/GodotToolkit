...
var _movement_velocity_applier: MovementVelocityApplier
...
func _find_sibling_components() -> void:
	...
	for child in _get_all_descendants(_character):
		...
		elif child is MovementVelocityApplier:
			_movement_velocity_applier = child
...
func _build_ui() -> void:
	...
	if _movement_velocity_applier:
		_vbox.add_child(_make_section_header("Velocity"))
		_accel_slider = _add_slider("Acceleration", _movement_velocity_applier.acceleration, 1.0, 200.0, 1.0,
			func(val: float) -> void: _movement_velocity_applier.acceleration = val
		)
		_decel_slider = _add_slider("Deceleration", _movement_velocity_applier.deceleration, 1.0, 200.0, 1.0,
			func(val: float) -> void: _movement_velocity_applier.deceleration = val
		)
		_air_control_slider = _add_slider("Air Control", _movement_velocity_applier.air_control, 0.0, 1.0, 0.01,
			func(val: float) -> void: _movement_velocity_applier.air_control = val
		)
		_vbox.add_child(_make_separator())
...
func _find_and_store_defaults() -> void:
	...
	if _movement_velocity_applier:
		_defaults["acceleration"] = _movement_velocity_applier.acceleration
		_defaults["deceleration"] = _movement_velocity_applier.deceleration
		_defaults["air_control"] = _movement_velocity_applier.air_control
	if _gravity_component:
...
func _reset_all_to_defaults() -> void:
	...
	if _movement_velocity_applier:
		if _defaults.has("acceleration"):
			_movement_velocity_applier.acceleration = _defaults["acceleration"]
			_accel_slider.value = _defaults["acceleration"]
		if _defaults.has("deceleration"):
			_movement_velocity_applier.deceleration = _defaults["deceleration"]
			_decel_slider.value = _defaults["deceleration"]
		if _defaults.has("air_control"):
			_movement_velocity_applier.air_control = _defaults["air_control"]
